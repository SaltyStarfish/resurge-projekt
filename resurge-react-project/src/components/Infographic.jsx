...
const VærdierBlocks = [
  { deskImgSrc: vaerdiboks1, mbImgSrc: lillevaerdiboks1, imageAlt: "visualisering af hvad vi tilbyder og hvores værdier", overskrift: "Optimal performance", text: "hurtige og effektive løsninger \n optimering med fokus på kvalitet", },
  { deskImgSrc: vaerdiboks2, mbImgSrc: lillevaerdiboks2, imageAlt: "visualisering af hvad vi tilbyder og hvores værdier", overskrift: "Skræddersyede løsninger", text: "bygger løsninger tilpasset hver kunde \n sikrer fuld kontrol", },
  { deskImgSrc: vaerdiboks3, mbImgSrc: lillevaerdiboks3, imageAlt: "visualisering af hvad vi tilbyder og hvores værdier", overskrift: "Konkurrencedygtige priser", text: "leverer høj værdi til fair priser \n fokuserer på effektiv og ærlig prissætning", },
  { deskImgSrc: vaerdiboks4, mbImgSrc: lillevaerdiboks4, imageAlt: "visualisering af hvad vi tilbyder og hvores værdier", overskrift: "Reduceret C02 - aftryk", text: "reducerer data og energiforbrug aktivt \n bygger effektive websites med lavere aftryk", }
];
//til ecra cera//
const CeraCera = [
...
export default function Infographic({ blocks }) {
  ...
  // Use CeraCera as default if no blocks prop is provided
  const shownBlocks = blocks && blocks.length > 0 ? blocks : CeraCera;
  //opdelt i mobil show og desk, fx deskImg eller mbImg//
  return (
    <div className="content-container w-full ">
      <h2 className="font-h2 font-medium w-full max-w-[694px] mb-8 text-left">CeraCera værdier</h2>
          ...
          {[0, 1].map(col => (
            <div key={col} className="flex flex-col gap-[10px] items-start mx-auto w-full max-w-[160px]">
              {shownBlocks.slice(col * 2, col * 2 + 2).map((block, idx) => (
                <div key={idx} className="relative group flex flex-col justify-between items-start mx-auto w-full max-w-[160px] min-h-[150px] h-[150px]">
                  <img src={block.mbImgSrc} alt={block.imageAlt} className="w-full max-w-[160px] h-[150px] object-cover rounded-[25px] absolute top-0 left-0 z-0" draggable="false" />
                  <div className={`relative z-10 w-full h-full flex flex-col justify-between px-2 py-3 ${block.textColor === "white" ? "text-white" : "text-black"}`}>
